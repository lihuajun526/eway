...
public class StringUtil {
     ...
     * a single String containing the provided elements.<br>
     * <br>
     * No delimiter is added before or after the list.
     * A {@code null} separator is the same as an empty String ("").<br>
     * <br>
     * Copied from Commons Lang, version 3 (r1138702).<br>
     *
    ...
    public static Set<String> splitToSet(String delimited, String separator) {
        ...
        return CollectionUtil.asSet(split);
    }
     ...
     * @param queryStringNameToRemove
     * @return
     */
    public static String removeQueryString(String originalQueryString, String queryStringNameToRemove){
		if(StringUtil.isEmpty(originalQueryString))
			return "";
		StringBuilder sb = new StringBuilder();
		String[] qStrs = originalQueryString.split("&");
		for(String qStr :qStrs){
			if(!qStr.contains(queryStringNameToRemove+"=")){
				if(sb.length()>0)
					sb.append("&");
				sb.append(qStr);
			}
    ...
    public static String truncateBytes(String src, int byteLength, String encoding){
    	if(src!=null){
    		try {
				byte[] byteContent = src.getBytes(encoding);
				if(byteContent.length>byteLength)
					src = new String(byteContent,0,byteLength,encoding);
